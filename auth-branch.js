(function () {
  ...
  async function loadSupabase() {
    ...
    client = client || window.supabase.createClient(CONFIG.url, CONFIG.anonKey, SUPABASE_AUTH_OPTIONS);
    return client;
  }
  ...
  function pendingDisplayName() {
    return (document.getElementById("authDisplayName")?.value || "").trim();
  }

  function requireRegisterName() {
    if (mode !== "register") return "";
    const displayName = pendingDisplayName();
    if (!displayName) throw new Error("Choose an account name before registering.");
    return displayName;
  }
  ...
  async function register(email, password, displayName) {
    ...
    const result = await supabaseClient.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: new URL("../ftlspd-portal.html", window.location.href).href,
  ...
  async function loginWithDiscord() {
    const supabaseClient = await loadSupabase();
    if (mode === "register") {
      const displayName = requireRegisterName();
      sessionStorage.setItem(PENDING_DISPLAY_NAME_KEY, displayName);
    }
  ...
  form?.addEventListener("submit", async function (event) {
    ...
    const displayName = mode === "register" ? pendingDisplayName() : "";
    ...
    try {
      const shouldRedirect = mode === "register" ? await register(email, password, displayName) : (await login(email, password), true);
      clearFailures();
      if (shouldRedirect) {
