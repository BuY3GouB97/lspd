(function () {
  ...
  const portalCopy = {
    en: {
      ...
      updateRegulation: "Update regulation",
      regulationDegree: "Degree",
      regulationCode: "Code (example 7-3)",
      regulationTitle: "Regulation title",
      regulationDescription: "Regulation description",
      savedRegulation: "Regulation saved.",
      confirmDeleteRegulation: "Delete this custom regulation?",
    ...
    ar: {
      ...
      updateRegulation: "تحديث اللائحة",
      regulationDegree: "الدرجة",
      regulationCode: "الكود (مثال 7-3)",
      regulationTitle: "عنوان اللائحة",
      regulationDescription: "وصف اللائحة",
      savedRegulation: "تم حفظ اللائحة.",
      confirmDeleteRegulation: "حذف هذه اللائحة المخصصة؟",
  ...
  function installStylesheet() {
    ensureStylesheet("ftlspd-portal.css?v=20260920c");
    ensureStylesheet("ftlspd-custom.css?v=20260920c");
  }
  ...
  function installRegulationManager() {
    ...
    header.insertAdjacentHTML("afterend", [
      ...
      '<input class="portal-input" name="regulation_code" data-i18n-placeholder="regulationCode" placeholder="' + safe(tr("regulationCode")) + '">',
      ...
      '<button id="regulationSaveButton" class="portal-button success" type="submit"><i class="fa-solid fa-plus"></i><span data-i18n="addRegulation">' + safe(tr("addRegulation")) + '</span></button>',
      '<button id="regulationCancelEditButton" class="portal-button ghost hidden" type="button"><i class="fa-solid fa-ban"></i><span data-i18n="cancel">' + safe(tr("cancel")) + '</span></button>',
      '</form>'
    ].join(""));
  }
  ...
  function bindPortalEvents() {
    ...
    document.getElementById("regulationCancelEditButton")?.addEventListener("click", cancelRegulationEdit);
  ...
  function buildFetchJobs(includeSecure) {
    ...
    jobs.push({
      ...
      timeout: 12000,
      query: function () {
        return state.client.from(tableName("regulations")).select("id,degree,regulation_code,title,description,created_by,created_at,updated_at").order("created_at", { ascending: false });
      },
      fallbackWhen: function (error) {
        return /regulation_code|schema cache|column/i.test(errorText(error));
      },
      fallbackQuery: function () {
  ...
  function normalizeRegulation(row) {
    return Object.assign({}, row, {
      ...
      regulation_code: row.regulation_code || row.code || "",
      title: row.title || "",
  ...
  async function submitApplication(event) {
    ...
    if (state.applicationSubmitting) return;
    state.applicationSubmitting = true;
    if (submitButton) submitButton.disabled = true;
    let submissionLockKey = "";
    let submitted = false;
    try {
      if (!validateApplicationStep()) return;
      ...
      const existingPending = getPendingApplication(state.applicationType, applicantEmail);
      if (existingPending) {
        toast(tr("pendingWarning", { type: existingPending.kind === "transfer" ? tr("transferLabel") : tr("recruitmentLabel") }), "warn");
        closeModal("applicationModal");
        return;
      }
      submissionLockKey = applicationSubmissionLockKey(state.applicationType, applicantEmail);
      if (isSubmissionLocked(submissionLockKey)) {
        toast(tr("pendingWarning", { type: state.applicationType === "transfer" ? tr("transferLabel") : tr("recruitmentLabel") }), "warn");
        return;
      }
      lockApplicationSubmission(submissionLockKey);
      ...
      if (submissionLockKey && !submitted) unlockApplicationSubmission(submissionLockKey);
      state.applicationSubmitting = false;
      if (submitButton) submitButton.disabled = false;
    }
  }

  function getPendingApplication(kind, email) {
    const normalizedEmail = String(email || "").trim().toLowerCase();
    return state.applications.find(function (app) {
      if (app.status !== "pending" || app.kind !== kind) return false;
      if (state.authUser?.id && app.applicant_user_id === state.authUser.id) return true;
      return normalizedEmail && String(app.applicant_email || "").toLowerCase() === normalizedEmail;
    });
  }

  function applicationSubmissionLockKey(kind, email) {
    const userPart = state.authUser?.id || String(email || "").trim().toLowerCase() || "guest";
    return "ftlspd-application-submit-lock:" + kind + ":" + userPart;
  }

  function isSubmissionLocked(key) {
    if (!key) return false;
    try {
  ...
  function lockApplicationSubmission(key) {
    try { if (key) sessionStorage.setItem(key, String(Date.now())); } catch (error) {}
  }

  function unlockApplicationSubmission(key) {
    try { if (key) sessionStorage.removeItem(key); } catch (error) {}
  }
  ...
  async function saveCustomRegulation(event) {
    event.preventDefault();
    if (!canManageCustomRegulations()) {
      toast(tr("noPermission"), "error");
      return;
    }
    const form = event.currentTarget;
    const data = Object.fromEntries(new FormData(form).entries());
    const editingId = state.regulationEditing;
    const existing = editingId ? state.customRegulations.find(function (entry) { return entry.id === editingId; }) : null;
    const patch = {
      degree: Number(data.degree),
      regulation_code: String(data.regulation_code || "").trim(),
      title: (data.title || "").trim(),
      description: (data.description || "").trim(),
      updated_at: new Date().toISOString()
    };
    if (!patch.title || !patch.description || !Number.isFinite(patch.degree)) {
      toast(tr("validationError"), "error");
      return;
    }
    const ok = existing
      ? await updateRow("regulations", existing.id, patch)
      : await insertRow("regulations", Object.assign({
        id: uid(),
  ...
  function updateRegulationFormState() {
    const form = document.getElementById("regulationForm");
    if (!form) return;
    const editing = Boolean(state.regulationEditing);
    form.classList.toggle("editing-asset", editing);
    const button = document.getElementById("regulationSaveButton");
    const cancel = document.getElementById("regulationCancelEditButton");
    if (button) {
      const icon = button.querySelector("i");
      const label = button.querySelector("span");
      if (icon) icon.className = "fa-solid " + (editing ? "fa-floppy-disk" : "fa-plus");
      if (label) {
        label.dataset.i18n = editing ? "updateRegulation" : "addRegulation";
        label.textContent = tr(editing ? "updateRegulation" : "addRegulation");
      }
    }
  ...
  window.editCustomRegulation = function (id) {
    if (!canManageCustomRegulations()) {
      toast(tr("noPermission"), "error");
      return;
    }
    const entry = state.customRegulations.find(function (item) { return item.id === id; });
    const form = document.getElementById("regulationForm");
    if (!entry || !form) return;
    state.regulationEditing = id;
    if (form.elements.degree) form.elements.degree.value = entry.degree;
    if (form.elements.regulation_code) form.elements.regulation_code.value = entry.regulation_code || "";
    if (form.elements.title) form.elements.title.value = entry.title || "";
    if (form.elements.description) form.elements.description.value = entry.description || "";
    updateRegulationFormState();
    form.scrollIntoView({ behavior: "smooth", block: "center" });
    toast(tr("editMode"), "success");
  };

  window.removeCustomRegulation = async function (id) {
    if (!canManageCustomRegulations()) {
      toast(tr("noPermission"), "error");
      return;
    }
    const entry = state.customRegulations.find(function (item) { return item.id === id; });
    if (!entry) return;
    if (!confirm(tr("confirmDeleteRegulation"))) return;
    const ok = await deleteRows("regulations", [id]);
    if (!ok) return;
    if (state.regulationEditing === id) cancelRegulationEdit();
    syncCustomRegulationsToRegistry();
    toast(tr("removed"), "success");
    await refreshAfterMutation();
  };
    ...
    try {
      ...
      state.customRegulations.map(normalizeRegulation).forEach(function (entry) {
        regulationsData.push({
          degree: entry.degree,
          id: entry.regulation_code || "IA-" + String(entry.id).slice(0, 8),
          title: entry.title,
      ...
      augmentCustomRegulationCards();
    } catch (error) {
      console.warn("LSPD regulation sync failed", error);
    }
  }

  function wrapRegulationRenderer() {
    try {
      if (typeof window.renderCards !== "function" || window.renderCards.__lspdWrapped) return;
      const original = window.renderCards;
      window.renderCards = function () {
        const result = original.apply(this, arguments);
        window.setTimeout(augmentCustomRegulationCards, 0);
        return result;
      };
      window.renderCards.__lspdWrapped = true;
    } catch (error) {
      console.warn("LSPD regulation renderer wrap failed", error);
    }
  }

  function augmentCustomRegulationCards() {
    if (!canManageCustomRegulations()) return;
    const cards = Array.from(document.querySelectorAll("#regulationsContainer .regulation-card"));
    state.customRegulations.map(normalizeRegulation).forEach(function (entry) {
      const code = entry.regulation_code || "IA-" + String(entry.id).slice(0, 8);
      const source = "custom:" + entry.id;
      const card = cards.find(function (node) {
        if (node.dataset.regulationSource === source) return true;
        const badge = node.querySelector(".code-badge");
        return badge && badge.textContent.trim() === "#" + code;
      });
      if (!card || card.querySelector(".custom-regulation-actions")) return;
      card.dataset.customRegulationId = entry.id;
      const actions = document.createElement("div");
      actions.className = "custom-regulation-actions";
      actions.innerHTML = '<button class="portal-button ghost" type="button" onclick="window.editCustomRegulation(\'' + safeAttr(entry.id) + '\')"><i class="fa-solid fa-pen-to-square"></i><span>' + safe(tr("edit")) + '</span></button><button class="portal-button danger" type="button" onclick="window.removeCustomRegulation(\'' + safeAttr(entry.id) + '\')"><i class="fa-solid fa-trash-can"></i><span>' + safe(tr("remove")) + "</span></button>";
  ...
  function auditPayload(value) {
    const allowed = [
      ...
      "media_type", "asset_id", "reaction", "degree", "regulation_code", "description", "badge_number", "rank", "department",
  ...
  function canManageCustomRegulations() {
    return canAddRegulations();
  }
