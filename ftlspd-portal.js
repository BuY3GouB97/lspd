(function () {
  ...
  function installStylesheet() {
    ensureStylesheet("ftlspd-portal.css?v=20260919d");
    ensureStylesheet("ftlspd-custom.css?v=20260919d");
  }
  ...
  function buildFetchJobs(includeSecure) {
    ...
    jobs.push({
      ...
      timeout: 12000,
      query: function () {
        return state.client.from(tableName("assets")).select(assetColumns).order("created_at", { ascending: false });
      },
      fallbackWhen: function (error) {
        return /storage_path|schema cache|column/i.test(errorText(error));
      },
      fallbackQuery: function () {
        return state.client.from(tableName("assets")).select("id,category,title,subtitle,discord_id,photo_url,caption,display_order,media_type,created_by,created_at,updated_at").order("created_at", { ascending: false });
      },
      apply: function (rows) {
