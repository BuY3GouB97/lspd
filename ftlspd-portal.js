(function () {
  ...
  function installStylesheet() {
    ensureStylesheet("ftlspd-portal.css?v=20260919c");
    ensureStylesheet("ftlspd-custom.css?v=20260919c");
  }
  ...
  async function fetchAll() {
    ...
    try {
      ...
      let accounts = { data: [], error: null };
      if (state.authUser && canAdmin()) {
        accounts = await state.client.from(tableName("accounts")).select(accountColumns).order("email", { ascending: true });
      } else if (state.authUser) {
        accounts = await state.client.from(tableName("accounts")).select(accountColumns).eq("auth_user_id", state.authUser.id).order("email", { ascending: true });
      }
      ...
      let assets = await state.client.from(tableName("assets")).select(assetColumns).order("created_at", { ascending: false });
      if (assets.error && /storage_path|schema cache|column/i.test(String(assets.error.message || assets.error.details || assets.error.code || ""))) {
        assets = await state.client.from(tableName("assets")).select("id,category,title,subtitle,discord_id,photo_url,caption,display_order,media_type,created_by,created_at,updated_at").order("created_at", { ascending: false });
      }
      const mediaReactions = await state.client.from(tableName("mediaReactions")).select("id,asset_id,reaction,user_key,user_email,created_at").order("created_at", { ascending: false });
      const customRegulations = await state.client.from(tableName("regulations")).select("id,degree,title,description,created_by,created_at,updated_at").order("created_at", { ascending: false });
      const schedule = await state.client.from(tableName("schedule")).select("id,badge_number,name,insignia_url,rank,department,admin_rank,status,punishment,last_promotion,discord_user,points,privilege_points,wings,created_by,created_at,updated_at").order("created_at", { ascending: true });
      const streams = await state.client.from(tableName("streams")).select("id,name,logo_url,kick_url,created_by,created_at,updated_at").order("created_at", { ascending: false });
      if (assets.error) throw assets.error;
      if (apps.error) console.warn(apps.error);
      if (accounts.error && /display_name|discord_user|discord_avatar_url|discord_provider_id|discord_linked|schema cache|column/i.test(String(accounts.error.message || accounts.error.details || accounts.error.code || ""))) {
