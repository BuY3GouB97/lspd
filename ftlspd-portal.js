(function () {
  ...
  const portalCopy = {
    en: {
      ...
      roleManagementText: "Owner only. Assign admin, FTO, media, IA, or applicant access to registered accounts.",
      profileTitle: "Complete Account Profile",
      profileSubtitle: "Add the account name and Discord user before using command systems.",
    ...
    ar: {
      ...
      roleManagementText: "للمالك فقط. عيّن صلاحيات أدمن أو FTO أو إعلام أو IA أو متقدم للحسابات المسجلة.",
      profileTitle: "إكمال ملف الحساب",
      profileSubtitle: "أضف اسم الحساب ومستخدم الديسكورد قبل استخدام أنظمة القيادة.",
  ...
  function scheduleViewHtml() {
    return [
      ...
      '</form>',
      '<div class="schedule-table-wrap"><table class="schedule-table"><thead><tr><th data-i18n="badgeNumber">' + safe(tr("badgeNumber")) + '</th><th data-i18n="name">' + safe(tr("name")) + '</th><th>INSIGNIA</th><th data-i18n="rank">' + safe(tr("rank")) + '</th><th data-i18n="department">' + safe(tr("department")) + '</th><th data-i18n="adminRank">' + safe(tr("adminRank")) + '</th><th data-i18n="status">' + safe(tr("status")) + '</th><th data-i18n="punishment">' + safe(tr("punishment")) + '</th><th data-i18n="lastPromotion">' + safe(tr("lastPromotion")) + '</th><th data-i18n="discordUser">' + safe(tr("discordUser")) + '</th><th data-i18n="points">' + safe(tr("points")) + '</th><th data-i18n="privilegePoints">' + safe(tr("privilegePoints")) + '</th><th data-i18n="wings">' + safe(tr("wings")) + '</th><th class="schedule-admin-col"></th></tr></thead><tbody id="scheduleTableBody"></tbody></table></div>',
      '</section>'
    ].join("");
  }
  ...
  function profileModalHtml() {
    return [
      ...
      '<label><span data-i18n="discordUserRequired">' + safe(tr("discordUserRequired")) + '</span><input id="profileDiscordInput" class="portal-input" type="text" required></label>',
      '</div>',
      '<div class="modal-actions"><button class="portal-button success" type="submit"><i class="fa-solid fa-floppy-disk"></i><span data-i18n="saveProfile">' + safe(tr("saveProfile")) + '</span></button></div>',
      '</form>',
      '</div>'
    ].join("");
  }
  ...
  async function syncAuthUser(user) {
    ...
    let profile = await state.client
      ...
      .select("id,email,role,display_name,discord_user")
      .eq("auth_user_id", user.id)
      .maybeSingle();
    if (profile.error && /display_name|discord_user|schema cache|column/i.test(String(profile.error.message || profile.error.details || profile.error.code || ""))) {
    ...
    state.authUser = {
      ...
      display_name: profile.data?.display_name || meta.full_name || meta.name || meta.global_name || meta.user_name || "",
      discord_user: profile.data?.discord_user || meta.preferred_username || meta.user_name || meta.name || ""
    };
    state.accountProfile = profile.data ? normalizeAccount(Object.assign({}, profile.data, state.authUser)) : state.authUser;
  ...
  function requireAccountProfile() {
    if (!state.authUser || state.profilePromptShown) return;
    if (state.authUser.display_name && state.authUser.discord_user) return;
  ...
  async function saveAccountProfile(event) {
    ...
    if (!displayName || !discordUser) {
      toast(tr("validationError"), "error");
      return;
    }
    const account = state.accountProfile || state.accounts.find(function (entry) { return entry.email === state.authUser.email || entry.auth_user_id === state.authUser.id; });
    if (!account || !account.id) {
      toast(tr("databaseError"), "error");
      return;
    }
    const ok = await updateRow("accounts", account.id, {
      display_name: displayName,
      discord_user: discordUser,
      updated_at: new Date().toISOString()
    });
    if (!ok) return;
    state.authUser.display_name = displayName;
    state.authUser.discord_user = discordUser;
    state.accountProfile = Object.assign({}, state.accountProfile || {}, { display_name: displayName, discord_user: discordUser });
  ...
  function subscribeRealtime() {
    ...
    state.realtimeChannel = state.client.channel("lspd-portal-realtime")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: tableName("mediaReactions") }, fetchAll)
      .on("postgres_changes", { event: "*", schema: "public", table: tableName("regulations") }, fetchAll)
      .on("postgres_changes", { event: "*", schema: "public", table: tableName("schedule") }, fetchAll)
      .on("postgres_changes", { event: "*", schema: "public", table: tableName("presence") }, refreshPresence)
  ...
  async function fetchAll() {
    ...
    try {
      const apps = state.authUser
        ? await state.client.from(tableName("applications")).select("*").order("created_at", { ascending: false })
        : { data: [], error: null };
      let accounts = state.role === "owner"
        ? await state.client.from(tableName("accounts")).select("id,auth_user_id,email,role,display_name,discord_user,created_at,updated_at").order("email", { ascending: true })
        : { data: state.authUser ? [state.authUser] : [], error: null };
      const roles = state.role === "owner"
        ? await state.client.from(tableName("roles")).select("*").order("email", { ascending: true })
        : { data: [], error: null };
      const assets = await state.client.from(tableName("assets")).select("*").order("created_at", { ascending: false });
      const mediaReactions = await state.client.from(tableName("mediaReactions")).select("*").order("created_at", { ascending: false });
      const customRegulations = await state.client.from(tableName("regulations")).select("*").order("created_at", { ascending: false });
      const schedule = await state.client.from(tableName("schedule")).select("*").order("created_at", { ascending: true });
      const streams = await state.client.from(tableName("streams")).select("*").order("created_at", { ascending: false });
      if (assets.error) throw assets.error;
      if (apps.error) console.warn(apps.error);
      if (accounts.error && /display_name|discord_user|schema cache|column/i.test(String(accounts.error.message || accounts.error.details || accounts.error.code || ""))) {
        accounts = await state.client.from(tableName("accounts")).select("id,auth_user_id,email,role,created_at,updated_at").order("email", { ascending: true });
      }
      if (accounts.error) console.warn(accounts.error);
      if (roles.error) console.warn(roles.error);
      if (mediaReactions.error) console.warn(mediaReactions.error);
      if (customRegulations.error) console.warn(customRegulations.error);
      if (schedule.error) console.warn(schedule.error);
      state.applications = apps.error ? [] : (apps.data || []).map(normalizeApplication);
      state.accounts = accounts.error ? [] : (accounts.data || []).map(normalizeAccount);
      state.roles = roles.data || [];
      state.assets = (assets.data || []).map(normalizeAsset);
      if (!mediaReactions.error) state.mediaReactions = (mediaReactions.data || []).map(normalizeMediaReaction);
      if (!customRegulations.error) state.customRegulations = (customRegulations.data || []).map(normalizeRegulation);
      if (!schedule.error) state.schedule = (schedule.data || []).map(normalizeScheduleRow);
  ...
  function renderDashboard() {
    ...
    document.getElementById("dashboardRoleButton")?.classList.toggle("hidden", state.role !== "owner");
  }
  ...
  function renderSchedule() {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="14" class="schedule-empty">' + safe(tr("scheduleEmpty")) + "</td></tr>";
      return;
    }
    body.innerHTML = rows.map(function (row) {
      const wings = scheduleWings.map(function (wing) {
        return '<label class="wing-check"><input type="checkbox" disabled ' + (row.wings && row.wings[wing[0]] ? "checked" : "") + '><span>' + safe(wing[1]) + '</span></label>';
      }).join("");
      ...
      return [
        ...
        '<td><div class="schedule-wings">' + wings + '</div></td>',
  ...
  function renderRoleList() {
    ...
    target.innerHTML = state.accounts.map(function (entry) {
      const demote = entry.role === "owner" ? "" : '<button class="portal-button danger" type="button" onclick="window.removeRole(\'' + safeAttr(entry.email) + '\')"><i class="fa-solid fa-user-minus"></i><span data-i18n="remove">' + safe(tr("remove")) + "</span></button>";
      const profile = [entry.display_name, entry.discord_user].filter(Boolean).join(" · ");
      return '<article class="review-card"><div class="review-top"><div><h3 class="review-title">' + safe(entry.email) + '</h3><div class="review-meta">' + safe(roleLabel(entry.role)) + (profile ? " · " + safe(profile) : "") + '</div></div>' + demote + '</div></article>';
    }).join("");
  }
  ...
  async function saveAsset(event, category) {
    ...
    const fileUrl = category === "media" ? await readSelectedFile(form.elements.media_file) : "";
    const photoUrl = (fileUrl || data.photo_url || "").trim();
  ...
  function auditPayload(value) {
    const allowed = [
      ...
      "role", "display_name", "discord_user", "category", "title", "subtitle", "discord_id", "caption", "display_order",
  ...
  async function refreshPresence() {
    ...
    try {
      ...
      const cutoff = new Date(now.getTime() - 90000).toISOString();
